...
Rewrite Rules - To use the tag search feature with your mod_rewrite rules (assuming you are using permalinks - ignore this if not) you should
			add the following to your .htaccess file, preferably below the "# END WordPress" line:
...
define('KEYWORDS_TAGURL', 'tag');								// URL to use when querying tags (need to add rewrite rules for this)
	...
	if ($include_cats) {
		...
		foreach($categories as $category) {
			...
			if ($linkmode == 'technorati')
				$taglink = KEYWORDS_TECHNORATI . "/" . urlencode($keyword);
			else
		...
		foreach($keywordlist as $keyword) {
			...
			if (!empty($keyword)) {
				switch ($linkmode) {
					...
					case 'search':
						if (KEYWORDS_REWRITEON)
							$taglink = get_settings('home') . "/" . KEYWORDS_SEARCHURL . "/" . urlencode($keyword);
						else
